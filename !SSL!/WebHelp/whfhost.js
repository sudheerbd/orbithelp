...
var gsTotalNumberOfSearchResults = "Total Number of Search Results : %1";
var gbShowTotalNumberOfSearchResults=1;
var gbEnableAndSearch =false;
...
function SearchPendingRequest()
{
    var oMsg=new whMessage(WH_MSG_SHOWFTS,null,null);
	notify(oMsg);
	oMsg = new whMessage(WH_MSG_GETSEARCHSTR, null, null);
	request(oMsg, onGetSearchStr);
}

function onGetSearchStr(oMsg)
{
    var strTerms = "";
    strTerms = oMsg.oParam;		
	if (strTerms != " " )
	{
		oMsg = new whMessage(WH_MSG_SEARCHTHIS, strTerms, null);		
		notify(oMsg);
	}	
}
...
{
	if(!gbAIRSSL)
		registerListener2(WH_MSG_SEARCHFTSKEY );
}

function FtsWriteClassStyle()
{
	var marginHoriZontal = (gsPageDir == "rtl")?"margin-right":"margin-left";
	var floatDir = (gsPageDir == "rtl")?"right":"left";
	var borderStyleright = (gsPageDir == "rtl")?"none":"solid";
	var borderStyleleft = (gsPageDir == "rtl")?"solid":"none";
	var directionHoriZontal = (gsPageDir == "rtl")?"right":"left";
	...
	sStyle+="P.msg {"+getFontStyle(goErrFont)+ marginHoriZontal + ":"+gsIndent+";margin-top:"+gsMargin+";}\n";
	sStyle+="dl {"+getFontStyle(goFont)+ marginHoriZontal + ":"+gsIndent+";margin-top:"+gsMargin+";}\n";
	...
	sStyle+="LI.sl_page{display:block;display:inline-block;float:" + floatDir +";cursor:pointer;text-decoration:none;border:1px solid #ccc;padding:.3em .7em;margin:0 .38em 0 0;text-align:center}\n";
	sStyle+="LI.sl_pageC{background:#f0f7e8;border:1px solid #83bc46;display:block;display:inline-block;float:" + floatDir +";cursor:pointer;text-decoration:none;padding:.3em .7em;margin:0 .38em 0 0;text-align:center}\n";
	...
	if(gbAIRSSL)
	    sStyle+="TD.fr_ts {padding:0.2em 0em;border-style:solid " + borderStyleright + " solid " + borderStyleleft + ";border-color:"+gsABgColor+";border-width:1px;}\n";
	else
	    sStyle+="TD.fr_ts {padding:0.2em 0em;border-style:none " + borderStyleright + " solid " + borderStyleleft + ";border-color:"+gsABgColor+";border-width:1px;}\n";
	if(gbAIRSSL)
	...
	sStyle+="P.pb_in {"+getFontStyle(goErrFont)+"position:relative;padding:0;margin:0;" + directionHoriZontal + ":0%;top:0%;height:10px;border-style:none;background-color:blue;}\n";
...
function window_Unload()
{
	if(!gbAIRSSL)
	{
		...
		unregisterListener2(WH_MSG_SEARCHFTSKEY);
		unregisterListener2(WH_MSG_SETMAXRSLT);
	}
}
...
function onReceiveNotification(oMsg)
{
    if(gbAIRSSL)
		return true;
	if(oMsg.msgId == WH_MSG_SEARCHFTSKEY )
	{
		if (oMsg.iParam)
		{
			...
			if( mrIsOnEngine()==true )
			{
				...
				g_sQuestion=oMsg.iParam;
				mrFindFTSKey();
			}else
			{
				// this is client side fts
				goOdinHunter.strQuery = oMsg.iParam.strQuery;
                gbExtraInfoInSearchQuery = oMsg.iParam.ExtraInfo;
				setTimeout( "Query()", 1 );
			};
		} 
	}
	else if ( oMsg.msgId == WH_MSG_PROJECTREADY )
    {
	    loadFts();
    }
    else if(oMsg.msgId == WH_MSG_SETMAXRSLT)
    {
        if(oMsg.iParam)
        {
            g_nMaxResult = oMsg.iParam;
        }   
    }
    return true;
}
...
function XmlReader()
{
	...
	this.loadFromFile = function(a_Context, a_funcCallback, a_bCache )
	{
		...
		xmlJsReader.loadFile(fileName, function(a_xmlDoc, args){
			    ...
			    if(gbAIRSSL)
			        setTimeout( function() { context.resume();}, 1);
			    else
...
function HuginPackageReader()
{
	...
	this.setSynonymForHighlighting = function( a_Context, a_this )
	{
		...
		if(!gbAIRSSL)
		{
			var oResMsg = new whMessage( WH_MSG_SETSYNSTR, strSyn, null );
			notify( oResMsg );
		}
		else
		{
...
function RunesService()
{
	...
	this.getTerm = function( a_Context, a_Rslt )
	{
		if ( this.langSev.isQuote( a_Context.getCurChar() ) )
		{
			...
			if(gbEnableAndSearch && gChangedQuery.length > 0 )
			{
				a_Context.strSrc = gChangedQuery;
			}
	...
	this.getLengthOfPhrase = function( a_str, a_nFrom )
	{
		...
		while ( i < nLen )
		{
			...
			{
				if(gbEnableAndSearch)
				{
					var nWordBreakLen = 0;
					nWordBreakLen = this.getLengthOfWordBreak( a_str, i+1 );
					
					var str = a_str.substring(i+nWordBreakLen, nLen);
				
					str = trim(str);

					if(str != "\"" && str.length >0)
						gChangedQuery = a_str.substring(0, i+1) + " \u00ACand\u00AC " + str;
				}
	...
	this.getLengthOfDefault = function( a_str, a_nFrom )
	{
				...
				!this.langSev.isQuote( a_str.charAt( i ) ) )
		{
			...
			{
				var nWordBreakLen = 0;
				nWordBreakLen = this.getLengthOfWordBreak( a_str, i );
				...
				if(str.length>0)				
					gChangedQuery = a_str.substring(0, i) + " \u00ACand\u00AC " + str;
	...
	**/
	...
	this.extractTerm = function( a_Context, a_Term )
	{
		...
		var termType = a_Term.eType ;
		
		this.dolSegment( a_Term );
		if ((termType == ESNT_NOT ) && ( a_Term.eType == ESNT_PHRASE))
			return false ; //change in term type to pharse means CJK search; not allowed for not operator
...
function HuginHunter()
{
	...
	{
        //In case of AIR, the AND search variable is set through other route
        if(!gbAIRSSL)
        {
            //To set the gbEnableAndSearch variable which enables the AND search
            SetVariablesFromNameValuePairString(gbExtraInfoInSearchQuery,false);
        }
		...
		for ( var i = 0; i < a_this.aRankedTopics.length; i++ )
		{
			for ( var j = 0; j < a_this.aRankedTopics[i].length; j++ )
			{
				...
				if(!(_isAbsPath(strTopicUrl) || _isRemoteUrl(strTopicUrl)))
				{
					if(gbAIRSSL)
					{
						a_this.queryResult.aTopics[nLen].strProjDir = a_this.aProjPathes[i].strProjDir;
					}
...
function onGetProjInfo(oMsg)
{
    if(oMsg && oMsg.oParam)
    {
		...
		if( aProj && aProj.length > 0 )
		{
			...
			{
                if(typeof(aProj[i].sFts) != 'undefined' && aProj[i].sFts != null){
				    goOdinHunter.aProjPathes[projIndx] = new Object();
				    goOdinHunter.aProjPathes[projIndx].strProjDir = aProj[i].sPPath;
				    goOdinHunter.aProjPathes[projIndx].strOdbPath = aProj[i].sPPath + aProj[i].sDPath + aProj[i].sFts;
                    projIndx++;
                }
			}
...
var gbAIRMergeArray;
function onGetMergeProjInfo(mergeProjInfo, strProjDir)
{
    goOdinHunter = new HuginHunter();
	goOdinHunter.aProjPathes = new Array();
	...
	if(mergeProjInfo!=null)
	{
		var index=0;
		while(index<mergeProjInfo.length)
		{
			goOdinHunter.aProjPathes[index+1] = new Object();
			goOdinHunter.aProjPathes[index+1].strProjDir = mergeProjInfo[index];
			goOdinHunter.aProjPathes[index+1].strOdbPath = mergeProjInfo[index] + "whxdata/whfts.xml";
			index++;
		}
...
function loadFts_context(strProjDir , strFtsFile)
{
	...
	{
		if(gbAIR)
			onGetMergeProjInfo(gbAIRMergeArray, strProjDir);
		else if(gbAIRSSL)
		{
		    if(isChromeLocal()){
		        this.parent.postMessage("getMergeProjInfo" + gMsgSeparator + " ", "*");
		    }
		    else{
			    if(this.parent.gbAIRmergeProjectArray != null)
				    onGetMergeProjInfo(this.parent.gbAIRmergeProjectArray, strProjDir);
		    }
		}
	}catch(e){}
}
...
function displayMsg( a_strMsg )
{
	...
	{
		//set num of results
		var oMsg = new whMessage(WH_MSG_SETNUMRSLT, 0, null);
		notify(oMsg);
	}
...
function displayTopics( a_QueryResult)
{
	...
	if ( a_QueryResult != null )
	{
		...
		{
			//set num of results
			var oMsg = new whMessage(WH_MSG_SETNUMRSLT, a_QueryResult.aTopics.length, null);
			notify(oMsg);
		}
		
        if (gbShowTotalNumberOfSearchResults && a_QueryResult.aTopics.length > 0)
        {
            var strTemp = gsTotalNumberOfSearchResults.replace(/%1/i, a_QueryResult.aTopics.length);
            sHTML = "<p class='msg'>" + strTemp + "</p>";
            sHTML +="<hr width=150% />";
        }
		...
		for( i = (g_CurPage-1)*nMaxResult; (i < a_QueryResult.aTopics.length)&&(i<(g_CurPage*nMaxResult)); i++ )
		{
			...
			if(gbAIRSSL) 
			{
				...
				else if(isValidType(a_QueryResult.aTopics[i].strProjDir))
				{
					//check if the length is greater than
					if(a_QueryResult.aTopics[i].strProjDir.length>0)
					{
						bAddHighLight = true;
					}
				}
...
function SetVariablesFromNameValuePairString(strNameValuePair,bDecodeValue)	//we need to decode in case of BB URL parameters 
{
    var nvPairs = strNameValuePair.split('&');
    ...
    for (i = 0; i < nvPairs.length; i++) {
		...
		if (window[name] != undefined) 
		{
			...
			{	
				window[name] = stringToBoolean(value);
			}
            if( typeof window[name] == 'boolean')
			{	
				window[name] = stringToBoolean(value);
			}
			else
...
function GetSearchTextFromURL()
{
	...
	strLocation = decodeURI(strLocation);

	var searchString = strLocation.substring(strLocation.indexOf('?') + 1 );
	
    SetVariablesFromNameValuePairString(searchString,true);

    return gsSearchQuery;
...
if ( gbAIRSSL || !window.gbTesting )
{
	...
	)
	{
		...
		if(!gbAIRSSL)
		{
			...
			registerListener2(WH_MSG_SETMAXRSLT);
		}
		gbWhFHost=true;
	}
	else
